...
const unsigned long OLED_DATA_SCREEN_3_DURATION_MS = 5000;
const unsigned long OLED_OFF_DURATION_MS = 10000;
...
enum OledDisplayState { OLED_STATE_ERROR_SCREEN, OLED_STATE_DATA_SCREEN_1, OLED_STATE_DATA_SCREEN_2, OLED_STATE_DATA_SCREEN_3, OLED_STATE_OFF };
...
const uint8_t  PORT_DECIM_N=10, PORT_BUF=24;
const float    PORT_TH_DP=3.5f, PORT_TH_DH=8.0f, PORT_TH_DIAQ=25.0f;
...
const float ALT_EMA_ALPHA = 0.12f;
const float ALT_OUTLIER_M = 18.0f;
const float ALT_DEADBAND_M = 0.40f;
...
const float HOT_ENTER_C=45.0f;
const float HOT_EXIT_C =41.0f;
const uint8_t CONTRAST_NORMAL=0x7F;
const uint8_t CONTRAST_OVERHEAT=0x04;
unsigned long nextSafetyProcessMs=0;
const unsigned long SAFETY_PERIOD_MS=60000;
uint8_t overheatPosIndex=0;

bool needInitialAltCal = true;
const float ALT_REF_M = 708.0f;
...
void getOverheatPos(uint8_t idx,int16_t &x,int16_t &y){
  switch(idx%5){ case 0: x=10;y=10;break; case 1: x=32;y=6;break; case 2: x=20;y=28;break; case 3: x=48;y=18;break; default: x=8;y=36;break; }
}
...
bool shouldRedrawScreen1(){
  ...
  if (isnan(prev_T)||isnan(prev_H)) return true;
...
bool shouldRedrawScreen2(){
  if (currentOledScreenState!=lastDrawnState) return true;
  if (isnan(prev_P)||isnan(prev_Alt)) return true;
...
void stampScreen1(){ prev_T=gTemp; prev_H=gHum; lastDrawnState=OLED_STATE_DATA_SCREEN_1; }
void stampScreen2(){ prev_P=gPress; prev_Alt=gAlt; lastDrawnState=OLED_STATE_DATA_SCREEN_2; }
void stampScreen3(){ prev_G=gGasEMA_kOhm; prev_IAQ=gIAQstaticDisp; prev_Acc=gIAQaccDisp; prev_AQS=iaqCategory(gIAQstaticDisp); lastDrawnState=OLED_STATE_DATA_SCREEN_3; }
...
void displaySensorDataScreen2() {
    ...
    // Perubahan di sini: format ke integer (%d) dan unit "mdpl"
...
void bsecLoopTick(){
  ...
  if (needInitialAltCal && isfinite(gPress)){
    float qnh_cal = qnhFromRef(gPress, ALT_REF_M);
    if (isfinite(qnh_cal) && qnh_cal>=870.0f && qnh_cal<=1100.0f){
      seaLevelPressure_hPa_current = qnh_cal;
      saveSeaLevelPressure(seaLevelPressure_hPa_current);
      needInitialAltCal = false;
    }
  }
  ...
  if (isnan(gAltSmooth)) gAltSmooth=alt_raw; else {
    float cand = ALT_EMA_ALPHA*alt_raw + (1.0f-ALT_EMA_ALPHA)*gAltSmooth;
    if (fabsf(cand - gAltSmooth) >= ALT_DEADBAND_M) gAltSmooth = cand;
  }
  gAlt=gAltSmooth;
  if (!bsecHasData) lastDrawnState=OLED_STATE_ERROR_SCREEN;
  bsecHasData=true;
  lastBsecDataMs=millis();
  if (isnan(gGasEMA_kOhm)) gGasEMA_kOhm=gGas_kOhm; else gGasEMA_kOhm=GAS_EMA_ALPHA*gGas_kOhm+(1.0f-GAS_EMA_ALPHA)*gGasEMA_kOhm;
  ...
  float aVar;
  if (vol<=2.0f) aVar=0.10f; else if (vol>=25.0f) aVar=0.45f; else aVar=0.10f+(vol-2.0f)*(0.35f/(25.0f-2.0f));
  if (gIAQacc>=3) aVar*=0.6f;
  if (gIAQacc<=1) aVar=fmaxf(aVar,0.28f);
  aVar=fminf(fmaxf(aVar,0.08f),0.45f);
  if (isnan(gIAQstaticDisp)) gIAQstaticDisp=gIAQstatic; else gIAQstaticDisp=aVar*gIAQstatic+(1.0f-aVar)*gIAQstaticDisp;
  if (millis()-lastBsecSaveMs>=BSEC_SAVE_INTERVAL_MS && gIAQacc>=3){ saveBsecState(); lastBsecSaveMs=millis(); }
  if (gIAQacc>gIAQaccPrev && gIAQacc>=2){ if (millis()-lastBsecSaveMs>=BSEC_MIN_SAVE_GAP_MS){ saveBsecState(); lastBsecSaveMs=millis(); } }
...
void readBME680SensorData(){
  ...
  if (thermal==THERM_NORMAL && gTemp>=HOT_ENTER_C){ enterHotHold(); return; }
  if (thermal==THERM_HOT_HOLD && gTemp<=HOT_EXIT_C){ exitHotHold(); }
  lastDrawnState=OLED_STATE_ERROR_SCREEN;
}
...
void updateOLEDDisplayContent(){
  ...
  switch(currentOledScreenState){
    ...
    case OLED_STATE_DATA_SCREEN_3:
      if (elapsed>=OLED_DATA_SCREEN_3_DURATION_MS){
        currentOledScreenState=OLED_STATE_OFF;
        oledScreenStateChangeMillis=now;
        oledCmd(SSD1306_DISPLAYOFF);
      }
      break;
    case OLED_STATE_OFF:
  ...
  } else if (currentOledScreenState==OLED_STATE_DATA_SCREEN_3){
    if (shouldRedrawScreen3()){ displaySensorDataScreen3(); stampScreen3(); }
  }
}
...
void handleSerialInput(){ while (Serial.available()){ char c=Serial.read(); (void)c; } }
